...
* Introduction
The statusline is an integral part of (neo)vim's interface. I'd always fancied making my own statusline, and galaxyline becoming unmaintained pushed me to make ithe switch.
...
- =:~= Reduce filename to be relative to the home directory, i.e. replace =/home/<user>= with =~=.
- =:.= Reduce filename to be relative to the current directory.
- =:h= Reduce filename to only the head (without this it would be printed twice.)
